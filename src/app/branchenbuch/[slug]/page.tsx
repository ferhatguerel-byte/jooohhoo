...
async function getCompany(slug: string) {
  ...
  const result = await db.query(
    `SELECT id, company_name, gewerke, plz, ort, verification_status, created_at
     FROM users
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical: `/branchenbuch/${buildCompanySlug(company.company_name, company.id)}` },
  }
}
